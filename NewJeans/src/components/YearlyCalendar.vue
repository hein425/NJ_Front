<template>
  <div class="calendar-wrapper">
    <div class="year-selector">
      ...
      <button @click="$router.push('/')" class="monthly-button">Monthly</button>
    ...
    <div class="calendar-grid">
      <div v-for="month in calendar" :key="month.month" class="month">
        ...
        <div class="grid">
          ...
          <div v-for="date in month.days" :key="date" class="date">{{ date }}</div>
        </div>
      </div>
    </div>
  </div>
...
const resetToCurrentYear = () => {
  currentYear.value = dayjs().year(); // 현재 연도로 설정
